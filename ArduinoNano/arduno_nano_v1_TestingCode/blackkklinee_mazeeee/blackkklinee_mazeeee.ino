...
void setup()
{
  ...
  while (s2 == HIGH)
  {
    s2 = digitalRead(sw2);      // Press s2 switch to start a bot to follow the path
  }
  delay(800);
}
void loop() {
...
void calibration()
{
  ...
  for (int i = 0; i <= 100; i++)
  {
    ...
    if (i < 25 || i >= 75)
    {
      left(motor1, motor2, 220); //Left turn

    }
    else
    {

      right(motor1, motor2, 220); //Right turn
    }
